...
const Navbar = () => {
  ...
  return (
    <div className="fixed flex justify-between top-0 z-10 h-[60px] w-full px-[20px] lg:px-[40px] bg-black bg-opacity-70">
            ...
            {checkRole(Role.Organizer) && (
              <div className="dropdown dropdown-end">
                <div
                  tabIndex={0}
                  role="button"
                  className="btn bg-opacity-0 text-white bg-zinc-900 text-[15px]"
                >
            ...
            {checkRole(Role.Experience) && (
              <div className="dropdown dropdown-end">
                <div className="flex items-center gap-[30px]">
                  <h1 className="flex items-center gap-1 text-white font-bold text-[15px]">
                    <Image src={coin} alt="coin" width={30} />
                    {(user.points > 0 ? user.points : 0).toLocaleString(
                      'id-ID',
                    )}
                  </h1>
                  <div>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-[250px]  bg-zinc-900  transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-20`}
      >
        ...
        <div className="flex flex-col p-4">
          {checkRole(Role.Organizer) && (
            <div className="flex flex-col gap-1 ">
              <h1 className="flex gap-3  items-center font-semibold text-white text-[20px] mb-4">
                <FiUser />
              ...
              <Link
                onClick={onLogout}
                href="/"
                className="flex justify-center font-semibold text-black bg-white rounded-full text-[15px] mb-4 w-1/3 p-1"
              >
                Log Out
              </Link>
            </div>
          ...
          {checkRole(Role.Experience) && (
            <div className="flex flex-col gap-1">
              ...
              <h1 className="flex items-center gap-2 pb-3 text-white font-medium">
                <Image src={coin} alt="coin" width={25} />
                {(user.points > 0 ? user.points : 0).toLocaleString('id-ID')}
              </h1>
              <Link
                href="/my-ticket"
                className="font-semibold text-white text-[15px] mb-4"
